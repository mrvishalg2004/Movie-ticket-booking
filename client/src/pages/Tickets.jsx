...
const Tickets = () => {
	...
	const fetchTickets = async () => {
		try {
			...
			setTickets(
				response.data.data.tickets?.sort((a, b) => (a.showtime.showtime > b.showtime.showtime ? 1 : -1))
			)
		} catch (error) {
			console.error(error)
		} finally {
						...
						{tickets.length === 0 ? (
							...
							<div className="grid grid-cols-1 gap-4 xl:grid-cols-2 min-[1920px]:grid-cols-3">
								{tickets.map((ticket, index) => (
									<div className="flex flex-col" key={index}>
										<ShowtimeDetails showtime={ticket.showtime} />
										<div className="flex h-full flex-col justify-between rounded-b-lg bg-gradient-to-br from-indigo-100 to-white p-4 text-center text-lg drop-shadow-lg md:flex-row">
											<div className="flex h-full flex-col items-center gap-2 px-4 md:flex-row">
												<p className="whitespace-nowrap font-semibold">Seats:</p>
												<div className="flex flex-wrap gap-2">
													{ticket.seats.map((seat, i) => (
														<div key={i} className="relative flex flex-col items-center">
															<img
																src="https://cdn-icons-png.freepik.com/512/4222/4222064.png"
																alt="Seat"
																className="w-10 h-10 drop-shadow-md"
															/>
															<p className="text-sm font-semibold text-gray-700">{seat.row + seat.number}</p>
														</div>
													))}
												</div>
												<p className="whitespace-nowrap text-gray-700">({ticket.seats.length} seats)</p>
											</div>
										</div>
									</div>
								))}
